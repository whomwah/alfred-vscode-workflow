...
async function fetchAndStore(config: Config, now: number) {
  const response = await fetchData(
    `${config.baseApiUrl}/repos/${REPO}/releases/latest`,
  );
  const releases: GhRelease = await response.json();

  config.latestVersion = releases.tag_name.replace("v", "");
  config.latestVersionLastChecked = now;
  storeConfig(config.db, "latestVersion", config.latestVersion);
  storeConfig(
    config.db,
    "latestVersionLastChecked",
    String(config.latestVersionLastChecked),
  );
}
